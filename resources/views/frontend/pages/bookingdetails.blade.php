...
@if($detail->status == 'pending')
        <td><a class="btn btn-danger" href="{{route('booking.delete',$detail->id)}}">Cancle</a></td>
  ...
  <div style="margin-right:150px; text-align:right;">
      @if($detail->status == 'pending')
         <h2>Total Amount:{{$detail->amount*$details->count()}}</h2>
         @else
         @endif
         <br>
      {{-- @if($detail->status == 'pending') --}}
         <a class="btn btn-primary" style="padding-right:80px;" href="{{route('user.payment',$detail->user_id)}}">Payment</a>
